...
class Search():
    ...
    def __init__(self, name):
        ...
        # search log
        self.d = {
            1 : list(itertools.product(range(self.sa1.shape[0]), range(self.sa1.shape[1]))),
            2 : list(itertools.product(range(self.sa2.shape[0]), range(self.sa2.shape[1]))),
            3 : list(itertools.product(range(self.sa3.shape[0]), range(self.sa3.shape[1]))),
        }
    ...
    def conduct_search(self, area_num, effectiveness_prob):
        ...
        if self.d[area_num]:
            self.d[area_num]
            random.shuffle(self.d[area_num])
            loc = int((len(self.d[area_num]) * effectiveness_prob)) if int((len(self.d[area_num]) * effectiveness_prob)) >= 1 else 1
            coords.extend(self.d[area_num][:loc])
            self.d[area_num] = self.d[area_num][loc:]
    ...
    def draw_menu(self,search_num):
        ...
        print(
            ...
            0 - Quit
            1 - Search Area 1 twice {0}
            2 - Search Area 2 twice {1}
            3 - Search Area 3 twice {2}
            4 - Search Areas 1 & 2  {0}
            5 - Search Areas 1 & 3  {1}
            6 - Search Areas 2 & 3  {2}
            7 - Start Over
            """.format("" if self.d[1] else "finished area 1","" if self.d[2] else "finished area 2","" if self.d[3] else "finished area 3")
            )
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            results_1, coords_1 = app.conduct_search(1, app.sep1)
            results_2, coords_2 = app.conduct_search(1, app.sep1)
            app.sep1 = (len(set(coords_1 + coords_2))) / (len(app.sa1)**2)
            app.sep2 = 0
            app.sep3 = 0

        elif choice == "2":
            results_1, coords_1 = app.conduct_search(2, app.sep2)
            results_2, coords_2 = app.conduct_search(2, app.sep2)
            app.sep1 = 0
            app.sep2 = (len(set(coords_1 + coords_2))) / (len(app.sa2)**2)
            app.sep3 = 0

        elif choice == "3":
            results_1, coords_1 = app.conduct_search(3, app.sep3)
            results_2, coords_2 = app.conduct_search(3, app.sep3)
            app.sep1 = 0
            app.sep2 = 0
